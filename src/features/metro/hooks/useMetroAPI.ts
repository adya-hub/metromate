...
export function useMetroSearch() {
  ...
  const search = useCallback((query: string) => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        const stops = await searchStops(query, 8, "metro");
...
export function useMetroPlanner() {
  ...
  const findRoute = useCallback(async (fromId: string, toId: string) => {
    ...
    try {
      const result = await planRoute(fromId, toId, "metro");
      console.log(
...
export function useMetroNearby() {
  ...
  const fetchNearby = useCallback(async (lat: number, lon: number, radius = 500) => {
    setIsLoading(true);
    try {
      const data = await getNearbyStops(lat, lon, radius, 6, "metro");
